...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  async refresh(refreshToken, { exchangeBody, clientAssertionPayload, DPoP, headers } = {}) {
    ...
    const tokenset = await this.grant(
      ...
      { clientAssertionPayload, DPoP, headers },
    );
  ...
  async grant(body, { clientAssertionPayload, DPoP, headers } = {}, retry) {
    ...
    try {
      ...
      if (retry !== retryAttempt && err instanceof OPError && err.error === 'use_dpop_nonce') {
        return this.grant(body, { clientAssertionPayload, DPoP, headers }, retryAttempt);
      }
      throw err;
    }
  ...
  async introspect(token, hint, { introspectBody, clientAssertionPayload, headers } = {}) {
    ...
    const response = await authenticatedPost.call(
      this,
      'introspection',
      { form, responseType: 'json', headers },
      { clientAssertionPayload },
    );
  ...
  async pushedAuthorizationRequest(params = {}, { clientAssertionPayload, headers } = {}) {
